...
def main() -> int:
    ...
    if not baseline_path.exists():
        ...
        summary["errors"].append(f"baseline file not found: {baseline_path}")
    ...
    if baseline.get("status") != "captured":
        ...
        summary["errors"].append(
            f"baseline status is {baseline.get('status')!r}; nothing to restore"
        )
        _write_summary(summary_path, summary)
        print(json.dumps({"status": summary["status"], "errors": summary["errors"]}))
        return 0
    ...
    baseline_profile = baseline.get("profile_name")
    if baseline_profile and baseline_profile != current_profile.name and os.environ.get(
        "DAST_CLEANUP_FORCE", "false"
    ).lower() != "true":
        summary["status"] = "refused"
        summary["errors"].append(
            f"baseline profile {baseline_profile!r} does not match current profile "
            f"{current_profile.name!r}; refusing to run. "
    ...
    try:
        with engine.begin() as conn:
            ...
            counts["matches_restored"] = restored_matches
            ...
            counts["match_audit_rows_deleted"] = deleted_audits
            ...
            counts["matches_deleted"] = deleted_matches

            baseline_classification_tx = {
            ...
            counts["categories_deleted"] = deleted_categories
            ...
            counts["categories_restored"] = restored_categories
