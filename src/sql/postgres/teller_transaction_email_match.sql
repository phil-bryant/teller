CREATE TABLE IF NOT EXISTS teller.transaction_email_match (
    ...
    state teller.transaction_email_match_state NOT NULL,
    ai_confidence DECIMAL(5,4) CHECK (ai_confidence IS NULL OR (ai_confidence >= 0 AND ai_confidence <= 1)),
    explanation_json JSONB NOT NULL DEFAULT '{}'::jsonb,
    selected_by teller.transaction_email_match_selected_by NOT NULL,
...
CREATE OR REPLACE FUNCTION teller.enforce_matchy_cardinality()
RETURNS TRIGGER AS $$
DECLARE
...
CREATE TRIGGER enforce_matchy_cardinality_trigger
    ...
    FOR EACH ROW
    EXECUTE FUNCTION teller.enforce_matchy_cardinality();

COMMENT ON TABLE teller.transaction_email_match IS 'Active and historical selected transaction-email links';
