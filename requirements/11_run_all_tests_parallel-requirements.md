...
Design: Discover child checks from `tests/NN_*.sh` files whose basenames contain `test` or `tests`.
Design: Keep `11_run_all_tests_parallel.sh` outside child discovery so the orchestrator never invokes itself.
...
Design: Child check scripts must not invoke or reference `11_run_all_tests_parallel.sh`; each child remains an independent numbered entrypoint.
Tests:
...
Design: Acquire a single-run lock file at repo-root scope before launching child checks and fail immediately if another live `11_run_all_tests_parallel.sh` process already owns the lock.
...
- 2026-05-20: Initial requirements for `11_run_all_tests_parallel.sh`.
