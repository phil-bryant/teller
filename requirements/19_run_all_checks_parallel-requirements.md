...
Applies to `19_run_all_checks_parallel.sh`.
...
R010  Statement: Run a fixed checklist of ten numbered check scripts.
Design: Launch, in order: `00_verify_requirements_traceability.sh`, `04_run_dependency_freshness_checks.sh`, `12_run_teller_api_smoke_checks.sh`, `05_run_av_checks.sh`, `06_run_sast.sh`, `08_verify_deploy_database.sh`, `09_run_unit_tests.sh`, `10_run_macos_ui_regression_tests.sh`, `11_verify_macos_crash_reporter.sh`, and `16_verify_classification_persistence.sh`.
...
Tests:
- R020-T01: Stub one child to exit non-zero and others to exit zero; verify all ten results are reported.

R025  Statement: Print per-script pass/fail summary lines as each check completes.
Design: Emit exactly ten `✅ PASS:` or `❌ FAIL:` lines, one per checklist script basename, in completion order as soon as each background job finishes (before the overall summary line).
Tests:
- R025-T01: When all children succeed, verify ten PASS lines.
...
Design: Exit `0` with an overall `✅ PASS:` line when all ten checks succeed; exit non-zero with an overall `❌ FAIL:` line when any check fails.
Tests:
...
Design: The ten child check scripts must not invoke or reference `19_run_all_checks_parallel.sh`; each child remains an independent numbered entrypoint.
Tests:
...
Design: Render a textual progress bar with completed/total counts and percentage that starts at `0/10 (0%)`, updates periodically while child checks are still running, and reaches `10/10 (100%)` before the final overall summary line.
...
Design: Acquire a single-run lock file at repo-root scope before launching child checks and fail immediately if another live `19_run_all_checks_parallel.sh` process already owns the lock.
...
- 2026-05-20: Initial requirements for `19_run_all_checks_parallel.sh`.
