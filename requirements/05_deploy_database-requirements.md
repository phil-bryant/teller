...
Design: Use `exec "${RUNNER_HOME}/06_deploy_database.sh" "$@"` so arguments pass through unchanged.
Tests:
- R015-T01: Verify wrapper source delegates to `06_deploy_database.sh` with `"$@"`.
