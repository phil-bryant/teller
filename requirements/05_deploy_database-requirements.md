# 05 deploy database Wrapper Requirements
...
Design: Call `delegate_golden "06_deploy_database.sh" "$@"` so the shim execs `${RUNNER_HOME}/06_deploy_database.sh` with arguments passed through unchanged.
Tests:
- R015-T01: Verify the pointer calls `delegate_golden "06_deploy_database.sh"` with `"$@"`.
