...
Design: Call `delegate_golden "tests/t08_run_fuzz_tests.sh" "$@"` so the shim execs `${RUNNER_HOME}/tests/t08_run_fuzz_tests.sh` with arguments passed through unchanged.
Tests:
- R015-T01: Verify the pointer calls `delegate_golden "tests/t08_run_fuzz_tests.sh"` with `"$@"`.
