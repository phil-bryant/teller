...
Tests:
- Execute DDL and verify `teller.audit_log` exists with expected columns and action constraint.
...
Tests:
- Call the function for a single-key table and a composite-key table and verify returned ordered column arrays.
...
Tests:
- Perform insert, update, and delete against a trigger-managed table and verify one audit row per operation with correct payload shape.
...
Tests:
- Audit events on a composite-key table and verify `record_id` stores all key parts.
...
Tests:
- Verify trigger creation excludes `audit_log` and includes other teller base tables.
