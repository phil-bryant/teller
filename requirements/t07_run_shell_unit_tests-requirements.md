...
Design: Use `exec "${RUNNER_HOME}/tests/t05_run_shell_unit_tests.sh" "$@"` so arguments pass through unchanged.
Tests:
- R015-T01: Verify wrapper source delegates to `tests/t05_run_shell_unit_tests.sh` with `"$@"`.
