...
Design: Use `exec "${RUNNER_HOME}/tests/t09_run_mutation_tests.sh" "$@"` so arguments pass through unchanged.
Tests:
- R015-T01: Verify wrapper source delegates to `tests/t09_run_mutation_tests.sh` with `"$@"`.
