...
Tests:
- Run `./10_run_teller-connect-ui.sh --add` and verify the page shows known local enrollments before connecting.
- Run `./10_run_teller-connect-ui.sh` with no local contexts and verify the page shows a no-enrollments message.
...
Tests:
- Run `./10_run_teller-connect-ui.sh` and verify existing rows expose reconnect/delete controls.
