...
Tests:
- Run helper calls against a test session and verify parameterized statements execute and returning rows are accessible.
...
Tests:
- Persist the same account payload twice and verify one logical account row remains with updated mutable fields.
...
Tests:
- Persist repeated owner payloads and verify identity rows are reused/updated rather than duplicated.
- Verify account-identity link insert is conflict-safe.
...
Tests:
- Persist transaction payload with links/details and verify relational rows and transaction row are written.
- Re-persist with changed mutable fields and verify transaction conflict update path applies.
...
Tests:
- Provide duplicate IDs with mixed statuses and verify canonical output keeps the posted variant.
...
Tests:
- Persist pending transactions, fetch a reduced set, and verify missing pending rows are deleted.
- Pass empty fetched IDs and verify all pending transactions for the account are removed.
...
Tests:
- Delete transactions that leave relation rows orphaned and verify orphan pruning removes only unreferenced rows.
...
Tests:
- Persist balances for an account and verify insert path writes links and balances.
- Re-persist with updated balances and verify update path and timestamp refresh.
...
Tests:
- Run `persist_all` with representative payloads and verify all data domains are persisted and committed.
- Verify `raw_balances_by_account` is optional and does not block persistence when omitted.
