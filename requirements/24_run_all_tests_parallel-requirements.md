...
Applies to `24_run_all_tests_parallel.sh`.
...
Design: Exclude `24_run_all_tests_parallel.sh` from discovery so the orchestrator never invokes itself.
...
Design: Child check scripts must not invoke or reference `24_run_all_tests_parallel.sh`; each child remains an independent numbered entrypoint.
Tests:
...
Design: Acquire a single-run lock file at repo-root scope before launching child checks and fail immediately if another live `24_run_all_tests_parallel.sh` process already owns the lock.
...
- 2026-05-20: Initial requirements for `24_run_all_tests_parallel.sh`.
