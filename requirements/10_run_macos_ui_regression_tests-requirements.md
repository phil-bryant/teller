...
Tests:
- R040-T01: Run `10_run_macos_ui_regression_tests.sh 1,3,5-6` and verify only matching `-only-testing` entries are passed to `xcodebuild test`.
...
Tests:
- R045-T01: Run `10_run_macos_ui_regression_tests.sh 99` and verify the script exits non-zero with an unknown-test-number error and does not call `xcodebuild`.
