# Run Swift Unit Tests Requirements

## Scope

Applies to `tests/t10_run_swift_unit_tests.sh`.
...
R005  Statement: Execute only the Swift unit-test lane.
Design: Invoke `src/scripts/run_unit_test_lanes.sh` with `RUN_SWIFT_TESTS=true` and all other unit lanes disabled.
Tests:
- R005-T01: Verify the wrapper exports lane toggles with only Swift tests enabled.
