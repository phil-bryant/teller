...
Design: Use `exec "${RUNNER_HOME}/98_destroy_database.sh" "$@"` so arguments pass through unchanged.
Tests:
- R015-T01: Verify wrapper source delegates to `98_destroy_database.sh` with `"$@"`.
