# 98 destroy database Wrapper Requirements
...
Design: Call `delegate_golden "98_destroy_database.sh" "$@"` so the shim execs `${RUNNER_HOME}/98_destroy_database.sh` with arguments passed through unchanged.
Tests:
- R015-T01: Verify the pointer calls `delegate_golden "98_destroy_database.sh"` with `"$@"`.
