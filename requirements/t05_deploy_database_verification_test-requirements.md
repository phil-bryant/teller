# t05 deploy database verification test Wrapper Requirements
...
Design: Use `exec "${RUNNER_HOME}/tests/t05_deploy_database_verification_test.sh" "$@"` so arguments pass through unchanged.
Tests:
- R015-T01: Verify wrapper source delegates to `tests/t05_deploy_database_verification_test.sh` with `"$@"`.
