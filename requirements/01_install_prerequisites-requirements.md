...
Design: End with success banner and example path references for `../1psa` and `../pg_install`, plus optional PLCrashReporter smoke verification entrypoint (`./11_verify_macos_crash_reporter.sh`, run separately—not from other numbered scripts).
Tests:
- R050-T01: On successful run, verify final message includes `../1psa` and `../pg_install`.
- R050-T02: On successful run, verify final guidance includes `./11_verify_macos_crash_reporter.sh`.
...
- 2026-05-12: R050 guidance references `./11_verify_macos_crash_reporter.sh` (standalone; not chained from other numbered scripts).
