...
Tests:
- Occupy the requested proxy port and verify the runner auto-selects a free fallback port.
- Set a non-numeric proxy port and verify startup fails with a clear validation error.
...
Tests:
- Run with defaults and verify quick-scan and daemon invocations use distinct ZAP home paths.
...
Tests:
- Stub daemon startup timeout and verify output includes failure context plus log tail marker.
