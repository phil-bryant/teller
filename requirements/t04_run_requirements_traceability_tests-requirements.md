...
Design: Use `exec "${RUNNER_HOME}/tests/t04_run_requirements_traceability_tests.sh" "$@"` so arguments pass through unchanged.
Tests:
- R015-T01: Verify wrapper source delegates to `tests/t04_run_requirements_traceability_tests.sh` with `"$@"`.
