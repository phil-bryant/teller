...
Design: Use `exec "${RUNNER_HOME}/97_backup_database.sh" "$@"` so arguments pass through unchanged.
Tests:
- R015-T01: Verify wrapper source delegates to `97_backup_database.sh` with `"$@"`.
