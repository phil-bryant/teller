...
Applies to `14_backfill_bank_statements.py`.
...
Tests:
- R001-T01: Provide sample PDF and verify OCR pipeline returns one text block per page.
...
Tests:
- R005-T01: Parse fixture pages and verify expected transaction count, dates, and signed amounts.
...
Tests:
- R010-T01: With multi-account institution and last-four hint, verify account match selection.
...
Tests:
- R015-T01: Verify generated IDs are stable for same transaction tuple and occurrence index.
...
Tests:
- R020-T01: Run with `--dry-run` and verify database writes are skipped.
...
- 2026-04-19: Initial reverse-engineered requirements for `14_backfill_bank_statements.py`.
