...
Applies to `11_report_quality_trends.sh`.
...
Design: If `quality-trend.json` does not exist under `QUALITY_TELEMETRY_DIR` (default `./artifacts/telemetry`), print a failure message to stderr that names the missing path and instructs operators to run `./10_run_all_tests_parallel.sh`.
Tests:
...
- 2026-05-26: Initial requirements for `11_report_quality_trends.sh`.
