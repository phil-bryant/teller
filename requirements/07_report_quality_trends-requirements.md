...
Design: Use `exec "${RUNNER_HOME}/12_report_quality_trends.sh" "$@"` so arguments pass through unchanged.
Tests:
- R015-T01: Verify wrapper source delegates to `12_report_quality_trends.sh` with `"$@"`.
