...
Design: Call `delegate_golden "08_report_quality_trends.sh" "$@"` so the shim execs `${RUNNER_HOME}/08_report_quality_trends.sh` with arguments passed through unchanged.
Tests:
- R015-T01: Verify the pointer calls `delegate_golden "08_report_quality_trends.sh"` with `"$@"`.
