...
Design: Use `exec "${RUNNER_HOME}/tests/t01_run_av_test.sh" "$@"` so arguments pass through unchanged.
Tests:
- R015-T01: Verify wrapper source delegates to `tests/t01_run_av_test.sh` with `"$@"`.
