# t01 run av test Wrapper Requirements
...
Design: Call `delegate_golden "tests/t01_run_av_test.sh" "$@"` so the shim execs `${RUNNER_HOME}/tests/t01_run_av_test.sh` with arguments passed through unchanged.
Tests:
- R015-T01: Verify the pointer calls `delegate_golden "tests/t01_run_av_test.sh"` with `"$@"`.
