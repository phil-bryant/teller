...
Design: Use `exec "${RUNNER_HOME}/99_restore_database.sh" "$@"` so arguments pass through unchanged.
Tests:
- R015-T01: Verify wrapper source delegates to `99_restore_database.sh` with `"$@"`.
