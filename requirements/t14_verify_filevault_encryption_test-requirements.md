...
Design: Call `delegate_golden "tests/t10_verify_filevault_encryption_test.sh" "$@"` so the shim execs `${RUNNER_HOME}/tests/t10_verify_filevault_encryption_test.sh` with arguments passed through unchanged.
Tests:
- R015-T01: Verify the pointer calls `delegate_golden "tests/t10_verify_filevault_encryption_test.sh"` with `"$@"`.
