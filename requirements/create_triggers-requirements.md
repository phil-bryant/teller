...
Tests:
- Update a row in a trigger-managed table and verify `updated_at` is changed by the trigger.
...
Tests:
- Add a teller table with `updated_at`, run script, and verify trigger creation for that table.
...
Tests:
- Re-run script and verify target table traversal order remains stable.
...
Tests:
- Execute script twice and verify no duplicate trigger error occurs on the second run.
...
Tests:
- Inspect `pg_trigger` metadata and verify each eligible table has the expected trigger name and function binding.
