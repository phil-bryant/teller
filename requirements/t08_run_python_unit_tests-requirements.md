# t08 run python unit tests Wrapper Requirements
...
Design: Call `delegate_golden "tests/t06_run_python_unit_tests.sh" "$@"` so the shim execs `${RUNNER_HOME}/tests/t06_run_python_unit_tests.sh` with arguments passed through unchanged.
Tests:
- R015-T01: Verify the pointer calls `delegate_golden "tests/t06_run_python_unit_tests.sh"` with `"$@"`.
