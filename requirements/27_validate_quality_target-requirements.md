...
Applies to `27_validate_quality_target.sh`.
...
Design: If `quality-history.ndjson` does not exist under `QUALITY_TELEMETRY_DIR` (default `./artifacts/telemetry`), print a failure message to stderr that names the missing path and instructs operators to run `./25_run_all_tests_parallel.sh` over time.
Tests:
...
- 2026-05-26: Initial requirements for `27_validate_quality_target.sh`.
