# 09 prune quality telemetry Wrapper Requirements

## Scope

Applies to `09_prune_quality_telemetry.sh`.
...
Design: Call `delegate_golden "10_prune_quality_telemetry.sh" "$@"` so the shim execs `${RUNNER_HOME}/10_prune_quality_telemetry.sh` with arguments passed through unchanged.
Tests:
- R015-T01: Verify the pointer calls `delegate_golden "10_prune_quality_telemetry.sh"` with `"$@"`.
