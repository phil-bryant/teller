# 08 validate quality target Wrapper Requirements

## Scope

Applies to `08_validate_quality_target.sh`.
...
Design: Call `delegate_golden "09_validate_quality_target.sh" "$@"` so the shim execs `${RUNNER_HOME}/09_validate_quality_target.sh` with arguments passed through unchanged.
Tests:
- R015-T01: Verify the pointer calls `delegate_golden "09_validate_quality_target.sh"` with `"$@"`.
