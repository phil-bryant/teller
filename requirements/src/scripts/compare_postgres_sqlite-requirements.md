...
Applies to `src/scripts/compare_postgres_sqlite.py`.
...
Tests:
- R001-T01: Run script with default profiles and verify both connections are attempted.
...
Tests:
- R005-T01: Introduce a table/column drift and verify the script reports it as a mismatch.
...
Tests:
- R010-T01: Modify one row value in either engine and verify the table is flagged with row mismatch details.
...
Tests:
- R015-T01: Run identical databases and verify exit `0`; run with drift and verify exit `1` plus report output.
