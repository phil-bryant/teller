...
LOCK_FILE="${SCRIPT_DIR}/.25_run_all_tests_parallel.lock"
...
acquire_single_run_lock() {
  ...
  if [[ -n "$existing_lock_pid" ]] && kill -0 "$existing_lock_pid" 2>/dev/null; then
    echo "❌ FAIL: another 25_run_all_tests_parallel.sh run is already active (pid ${existing_lock_pid})." >&2
    return 1
  fi
  rm -f "$LOCK_FILE"
  if ( set -o noclobber; echo "$$" > "$LOCK_FILE" ) 2>/dev/null; then
...
for script in "${CHECKS[@]}"; do
  ...
  (
    ...
    if [[ "$script" == "17_verify_macos_crash_test.sh" && "$crash_check_delay" =~ ^[0-9]+$ && "$crash_check_delay" -gt 0 ]]; then
      sleep "$crash_check_delay"
    fi
    if [[ "$script" == "09_deploy_database_verification_test.sh" || "$script" == "13_run_sql_unit_tests.sh" || "$script" == "22_classification_persistence_verification_test.sh" || "$script" == "23_run_dynamic_security_tests.sh" ]]; then
      if [[ "$script" == "22_classification_persistence_verification_test.sh" ]]; then
        ...
        TELLER_CLASSIFIER_API_URL="${TELLER_CLASSIFIER_API_URL:-${lane_api_url}}" \
          "./${script}" >"${log}" 2>&1
      elif [[ "$script" == "23_run_dynamic_security_tests.sh" ]]; then
        TELLER_DB_HOST="${TELLER_DB_HOST:-127.0.0.1}" \
        TELLER_DB_SSLMODE="${TELLER_DB_SSLMODE:-require}" \
