...
ensure_unique_suffix() {
    #region agent log
    debug_log "H1" "08_capture_teller_token.sh:ensure_unique_suffix" "entry" "raw_arg=$1"
    #endregion
...
usage() {
    ...
    echo "Usage:"
    echo "  ./08_capture_teller_token.sh                 # default: no copy/paste flow"
    echo "  ./08_capture_teller_token.sh --connect      # explicit no copy/paste flow"
    echo "  ./08_capture_teller_token.sh --manual"
    echo "  ./08_capture_teller_token.sh [token_xxx]"
    echo "  ./08_capture_teller_token.sh --clipboard"
    echo "  ./08_capture_teller_token.sh --list"
    echo "  ./08_capture_teller_token.sh --delete --institution_id <id>|--enrollment_id <id> [--yes]"
    echo "  ./08_capture_teller_token.sh --reconnect --institution_id <id>|--enrollment_id <id>"
    echo "  ./08_capture_teller_token.sh --add"
    echo ""
...
verify_accounts_access() {
    ...
    if [ "$status" = "200" ]; then
        ...
        if [[ "$error_code" == enrollment.disconnected* ]] && [ -z "$ENROLLMENT_ID" ]; then
            ...
            echo "  ENROLLMENT_ID=enr_xxx ./08_capture_teller_token.sh"
        fi
        echo "Response:"
        cat "$response_file"
    fi
...
EOF
        ...
        --add)
            ...
            #region agent log
            debug_log "H2" "08_capture_teller_token.sh:main:add" "temp_output_paths" "token_out=$token_out enrollment_out=$enrollment_out"
            #endregion
            run_connect_capture "" "$token_out" "$enrollment_out" "capture"
            institution="$(infer_institution_id_from_token_file "$token_out")"
            #region agent log
            debug_log "H3" "08_capture_teller_token.sh:main:add" "institution_from_identity" "institution=$institution"
            #endregion
            [ -n "$institution" ] || institution="$(tr -d '\r\n' < "$enrollment_out")"
            #region agent log
            debug_log "H4" "08_capture_teller_token.sh:main:add" "institution_after_fallback" "institution=$institution"
            #endregion
            institution="$(sanitize_suffix "$institution")"
            #region agent log
            debug_log "H5" "08_capture_teller_token.sh:main:add" "institution_after_sanitize" "institution=$institution"
            #endregion
