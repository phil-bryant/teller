...
overview: Eliminate or intentionally gate all warnings emitted by `04_run_dependency_freshness_checks.sh` by separating signal vs noise, enforcing explicit Teller auth input, and making PostgreSQL server-version detection deterministic in local/CI.
todos:
...
## Goal
Make `./04_run_dependency_freshness_checks.sh` produce actionable output with no ambiguous warnings during normal usage, while preserving safety checks.
...
- PostgreSQL lane cannot resolve server version when connection inputs are incomplete in [`scripts/check_postgres_freshness.py`](/Users/phil/local/src/teller/scripts/check_postgres_freshness.py) and wrapper defaults in [`04_run_dependency_freshness_checks.sh`](/Users/phil/local/src/teller/04_run_dependency_freshness_checks.sh).
...
2. **Make Teller auth explicit to remove ambiguity warnings**
   - In [`04_run_dependency_freshness_checks.sh`](/Users/phil/local/src/teller/04_run_dependency_freshness_checks.sh), enforce explicit auth input for Teller canary runs:
   ...
   - Update usage docs in [`requirements/04_run_dependency_freshness_checks-requirements.md`](/Users/phil/local/src/teller/requirements/04_run_dependency_freshness_checks-requirements.md).

3. **Keep Postgres server-version check enabled and make it deterministic**
   - Preserve `POSTGRES_CHECK_SERVER_VERSION=true` default in [`04_run_dependency_freshness_checks.sh`](/Users/phil/local/src/teller/04_run_dependency_freshness_checks.sh).
...
4. **Test coverage and regression checks**
   - Extend shell tests in [`tests/sh/04_run_dependency_freshness_checks.bats`](/Users/phil/local/src/teller/tests/sh/04_run_dependency_freshness_checks.bats) for:
   ...
   - Verify end-to-end by rerunning `./04_run_dependency_freshness_checks.sh` and confirming warning behavior matches policy.
