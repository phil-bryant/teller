...
overview: Remove the macOS XCUITest-through-ZAP-proxy lane from DAST scripts (primarily `16_run_dast.sh`), along with its requirements and bats tests. Keep standalone UI regression testing via `15_run_macos_ui_regression_tests.sh` unchanged.
todos:
  - id: strip-16-dast
...
3. **macOS UI DAST** — ZAP daemon proxy + `./15_run_macos_ui_regression_tests.sh` with `RUN_XCUITESTS=true` (this is what failed on the missing `activateTransactionClassifierForInput` symbol and what you want removed)
...
- `[15_run_macos_ui_regression_tests.sh](15_run_macos_ui_regression_tests.sh)` and all `macos-ui/` snapshot/XCUITest code
...
flowchart LR
  ...
  UI[XCUITest via 15_run_macos_ui_regression_tests.sh]
    API --> SCH
    API --> ZAPQ
    API --> ZAPD
    ZAPD --> UI
...
Delete the entire macOS UI DAST block (~lines 1386–1482): `RUN_MACOS_UI_DAST`, proxy host/port selection, ZAP `-daemon`, invocation of `15_run_macos_ui_regression_tests.sh`, and `zap-macos-ui.{json,html,log}` artifact collection.
...
- `[macos-ui/README.md](macos-ui/README.md)`: Remove line 88 (`RUN_SAST=false RUN_MACOS_UI_DAST=true ./16_run_dast.sh`); verification helpers should list only `./15_run_macos_ui_regression_tests.sh` for UI testing
