...
todos:
  ...
  - id: rewrite-xcuitests
    ...
    status: completed
  - id: update-runner
    content: "Update 10_run_macos_ui_regression_tests.sh: XCUITEST_SCENARIOS, testMacOSUISmokeSuite target, XCUITEST_STEPS env passthrough"
    status: completed
  - id: update-reqs-bats
...
`[10_run_macos_ui_regression_tests.sh](10_run_macos_ui_regression_tests.sh)` orchestrates two lanes correctly (snapshots pass), but the XCUITest lane is broken and obsolete:
...
```mermaid
sequenceDiagram
    participant Script as 10_run_macos_ui_regression_tests.sh
...
### 3. Update runner script — `[10_run_macos_ui_regression_tests.sh](10_run_macos_ui_regression_tests.sh)`
...
### 4. Update requirements — `[requirements/10_run_macos_ui_regression_tests-requirements.md](requirements/10_run_macos_ui_regression_tests-requirements.md)`
...
### 5. Update bats tests — `[tests/sh/10_run_macos_ui_regression_tests.bats](tests/sh/10_run_macos_ui_regression_tests.bats)`
...
2. `./10_run_macos_ui_regression_tests.sh` — snapshot + XCUITest green
3. `./10_run_macos_ui_regression_tests.sh 3,6` — runs subset via `XCUITEST_STEPS` without relaunching between steps
4. `bats tests/sh/10_run_macos_ui_regression_tests.bats` — all requirement-tagged tests pass
