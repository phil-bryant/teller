...
overview: Add macOS hover-help text to all interactive controls in the TransactionClassifier UI launched by `18_run_classification_macos-ui.sh`, then add XCUITest coverage so `10_run_macos_ui_regression_tests.sh` verifies tooltip display behavior.
todos:
  ...
  - id: add-hover-scenario
    ...
    status: completed
  - id: wire-10-runner
    content: Update `10_run_macos_ui_regression_tests.sh` scenario mapping/count if required for new hover scenario.
    status: completed
...
- Hover-verification in regression flow will be implemented in:
  ...
  - [`/Users/phil/local/src/teller/10_run_macos_ui_regression_tests.sh`](/Users/phil/local/src/teller/10_run_macos_ui_regression_tests.sh) (scenario list wiring if needed)
- Requirements traceability updates will be made in:
  ...
  - [`/Users/phil/local/src/teller/requirements/10_run_macos_ui_regression_tests-requirements.md`](/Users/phil/local/src/teller/requirements/10_run_macos_ui_regression_tests-requirements.md)
...
- Update `10_run_macos_ui_regression_tests.sh` scenario mapping if a new smoke step is introduced (and keep selector semantics `N`, `N,M`, `N-M` intact).

## Verification strategy (requested: UI hover assertions)
- Run the XCUITest smoke suite via `10_run_macos_ui_regression_tests.sh` with the hover-help step included.
