...
#R005: Extract statement year/month from OCR text using statement date marker.
...
#R005: Extract deposit/withdrawal summary totals and counts from OCR text.
...
#R005: Normalize OCR amount fragments to canonical decimal strings.
def _normalize_amount(raw):
    return f"0{raw}" if raw.startswith('.') else raw


#R005: Resolve the transaction amount field from grouped OCR lines.
...
#R005: Extract candidate activity lines from OCR pages after the activity table header.
def _extract_activity_lines(pages):
    all_lines = []
    for page in pages:
        in_section = False
...
#R005: Merge OCR artifacts where a date token and its description were split across lines.
def _merge_split_date_lines(all_lines):
    normalized_lines = []
    i = 0
...
#R005: Group normalized OCR lines into transaction buckets keyed by leading MM/DD tokens.
def _group_transaction_lines(normalized_lines):
    groups, current = [], None
...
#R005: Convert one grouped OCR transaction payload into a normalized transaction record.
def _transaction_from_group(group, year):
    raw_amount, amt_idx, trimmed = _find_amount(group['lines'])
    if raw_amount is None:
        log.warning("No amount found, skipping", lines=group['lines'][:2])
        return None
    desc_parts = []
    for line_index, line in enumerate(group['lines']):
        desc_parts.append(trimmed if line_index == amt_idx else line)
    description = ' '.join(part for part in desc_parts if part)
    amount_clean = raw_amount.replace(',', '')
    desc_upper = description.upper()
    is_cred = any(desc_upper.startswith(prefix) for prefix in CREDIT_PREFIXES) or any(
        token in desc_upper for token in CREDIT_CONTAINS
    )
    signed = amount_clean if is_cred else f"-{amount_clean}"
    return {
        'date': f"{year}-{group['month']:02d}-{group['day']:02d}",
        'amount': signed,
...
def parse_transactions(pages, year, stmt_month):
    ...
    all_lines = _extract_activity_lines(pages)
    normalized_lines = _merge_split_date_lines(all_lines)
    groups = _group_transaction_lines(normalized_lines)
    result = []
    for group in groups:
        record = _transaction_from_group(group, year)
...
#R005: Recover buried statement interest rows when OCR grouping misses the explicit transaction line.
...
#R005: Infer normalized transaction type from OCR description prefixes.
...
#R015: Build deterministic statement transaction IDs from account/date/amount/description/occurrence.
...
#R020: Resolve statement root from CLI override, env override, or repo-default path.
...
#R020: Resolve institution scope from CLI filter or teller.account inventory.
...
#R020: Discover statement PDFs for one institution under the statements root.
...
#R010: Load teller accounts for one institution to drive statement-to-account matching.
...
#R010: Extract statement account last-four hint from filename and OCR head text.
...
#R020: Configure structured logging at debug or info level based on CLI flags.
def _configure_logging(debug):
    structlog.configure(wrapper_class=structlog.make_filtering_bound_logger(
        logging.DEBUG if debug else logging.INFO))


#R025: Parse one statement PDF and emit parsing quality audit logs.
def _parse_statement_pdf(pdf_path, institution_id, account_rows, account_id_override):
    log.info("Processing PDF", institution_id=institution_id, path=str(pdf_path))
    pages = ocr_pdf(pdf_path)
    account_id = match_statement_to_account(pdf_path, pages, account_rows, account_id_override)
    year, month = extract_statement_year(pages)
    txns = parse_transactions(pages, year, month)
    _dep_count, dep_total, _wd_count, wd_total = extract_summary(pages)
    ...
    log.info(
        ...
        month=month,
        txn_count=len(txns),
        deposits=str(parsed_dep),
        withdrawals=str(parsed_wd),
        expected_dep=str(dep_total),
        expected_wd=str(wd_total),
    )
    #R025: Log parsed-vs-summary mismatches for audit visibility.
    if dep_total is not None and parsed_dep != dep_total:
        log.warning("Deposit total mismatch", institution_id=institution_id, parsed=str(parsed_dep), expected=str(dep_total))
    if wd_total is not None and parsed_wd != wd_total:
        log.warning("Withdrawal total mismatch", institution_id=institution_id, parsed=str(parsed_wd), expected=str(wd_total))
    return account_id, txns


#R015: Persist one account slice with deterministic IDs while skipping API-overlap dates.
def _persist_account_transactions(session, institution_id, account_id, all_txns, dry_run, upsert_transaction):
...
#R025: Process one institution from PDF discovery through account-slice persistence.
def _process_institution(session, root, institution_id, account_id_override, dry_run, upsert_transaction):
    ...
    for pdf_path in pdfs:
        account_id, txns = _parse_statement_pdf(pdf_path, institution_id, account_rows, account_id_override)
        by_account.setdefault(account_id, []).extend(txns)
    parsed_total = sum(len(value) for value in by_account.values())
    inserted_total, skipped_total = 0, 0
    for account_id, all_txns in sorted(by_account.items(), key=lambda item: item[0]):
        log.info("Persisting account slice", institution_id=institution_id, account_id=account_id, txn_count=len(all_txns))
        inserted_count, skipped_count = _persist_account_transactions(
            session=session,
            institution_id=institution_id,
            account_id=account_id,
            all_txns=all_txns,
    ...
    return inserted_total, skipped_total, parsed_total
...
def main():
    ...
    try:
        ...
        for institution_id in institution_ids:
            account_override = args.account_id if args.institution_id == institution_id else None
            inserted_count, skipped_count, parsed_count = _process_institution(
                session=session,
                root=root,
                institution_id=institution_id,
