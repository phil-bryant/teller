...
# Requirement test-case tags for requirements/15_run_macos_ui_regression_tests-requirements.md
# #R010-T02: Traceability anchor.
# #R020-T02: Traceability anchor.

# Traceability numbered tags for requirements/15_run_macos_ui_regression_tests-requirements.md
...
EOF
  ...
  run bash -c "cd '${FIXTURE_ROOT}' && \
    export PATH='${STUB_BIN}:'\${PATH} && \
    RUN_SNAPSHOT_TESTS=false RUN_XCUITESTS=true \
    ./15_run_macos_ui_regression_tests.sh 1,3,5-6"
  [ "$status" -eq 0 ]
  ...
  run bash -c "cd '${FIXTURE_ROOT}' && \
    export PATH='${STUB_BIN}:'\${PATH} && \
    RUN_SNAPSHOT_TESTS=false RUN_XCUITESTS=true \
    ./15_run_macos_ui_regression_tests.sh 12"
  [ "$status" -eq 0 ]
  grep -F "testMacOSUISmokeSuite" "${CALLS_LOG}"
...
EOF
  ...
  run bash -c "cd '${FIXTURE_ROOT}' && \
    export PATH='${STUB_BIN}:'\${PATH} && \
    RUN_SNAPSHOT_TESTS=false RUN_XCUITESTS=true \
    ./15_run_macos_ui_regression_tests.sh 99"
  [ "$status" -ne 0 ]
