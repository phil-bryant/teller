...
setup() {
  setup_shell_test
  create_repo_fixture
  copy_script_to_fixture "08_capture_teller_token.sh"
  cat > "${FIXTURE_ROOT}/teller_connect_token_server.py" <<EOF
#!/usr/bin/env bash
echo "capture-server \$*" >> "${CALLS_LOG}"
exit 0
EOF
  chmod +x "${FIXTURE_ROOT}/teller_connect_token_server.py"
}
...
@test "default mode launches capture server in manage mode" {
  run bash "${FIXTURE_ROOT}/08_capture_teller_token.sh"
  [ "$status" -eq 0 ]
  calls="$(<"${CALLS_LOG}")"
...
@test "list mode prints discovered contexts" {
  ...
  run bash "${FIXTURE_ROOT}/08_capture_teller_token.sh" --list
  [ "$status" -eq 0 ]
  [[ "$output" == *"source"* ]]
  [[ "$output" == *"enr_default"* ]]
  [[ "$output" == *"bank_a"* ]]
...
@test "delete mode removes only selected context using --yes" {
  ...
  run bash "${FIXTURE_ROOT}/08_capture_teller_token.sh" --delete --institution_id bank_a --yes
  [ "$status" -eq 0 ]
...
@test "positional token writes auth_token.json with restrictive mode" {
  run bash "${FIXTURE_ROOT}/08_capture_teller_token.sh" token_direct_value
  [ "$status" -eq 0 ]
...
@test "selector operations fail clearly when selector is ambiguous" {
  ...
  run bash "${FIXTURE_ROOT}/08_capture_teller_token.sh" --delete --enrollment_id enr_shared
  [ "$status" -eq 0 ]
  [[ "$output" == *"ambiguous"* ]]
}
