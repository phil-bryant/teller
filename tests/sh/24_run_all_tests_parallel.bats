...
# Requirement test-case tags for requirements/24_run_all_tests_parallel-requirements.md
...
# Traceability numbered tags for requirements/24_run_all_tests_parallel-requirements.md
...
setup() {
  setup_shell_test
  create_repo_fixture
  copy_script_to_fixture "24_run_all_tests_parallel.sh"
...
@test "runs from repository root regardless of caller directory" {
  ...
  run bash -c "cd '${TEST_TMPDIR}' && PARALLEL_CHECKS_REPORT_DIR='${REPORT_DIR}' bash '${FIXTURE_ROOT}/24_run_all_tests_parallel.sh'"
  [ "$status" -eq 0 ]
...
@test "discovers only numbered test scripts and excludes self" {
  ...
  [[ "$output" != *"✅ PASS: 24_run_all_tests_parallel.sh"* ]]
...
@test "waits for all checks and reports a single failed child" {
  ...
  [[ "$output" == *"see ${REPORT_DIR}/10_run_python_unit_tests.log"* ]]
...
@test "rejects concurrent orchestrator runs with an active lock" {
  ...
  env PARALLEL_CHECKS_REPORT_DIR="${REPORT_DIR}" \
    bash "${FIXTURE_ROOT}/24_run_all_tests_parallel.sh" > "${TEST_TMPDIR}/first-run.log" 2>&1 &
  first_pid="$!"
  sleep 0.2
...
@test "reclaims stale lock file and succeeds" {
  #R050
  write_all_child_stubs 'exit 0'
  printf '%s\n' 999999 > "${FIXTURE_ROOT}/.24_run_all_tests_parallel.lock"

  run env PARALLEL_CHECKS_REPORT_DIR="${REPORT_DIR}" \
    bash "${FIXTURE_ROOT}/24_run_all_tests_parallel.sh"
  [ "$status" -eq 0 ]
  [ ! -f "${FIXTURE_ROOT}/.24_run_all_tests_parallel.lock" ]
}
