...
setup() {
  setup_shell_test
  create_repo_fixture
  copy_script_to_fixture "04_load_requirements.sh"
}
...
@test "fails when expected venv directory is missing" {
  #R001-T01
  run bash -c "cd '${FIXTURE_ROOT}' && ./04_load_requirements.sh"
  [ "$status" -eq 1 ]
...
@test "fails when no virtual environment is active" {
  ...
  run bash -c "cd '${FIXTURE_ROOT}' && unset VIRTUAL_ENV && ./04_load_requirements.sh"
  [ "$status" -eq 1 ]
...
@test "fails when active venv does not match project venv" {
  ...
  run bash -c "cd '${FIXTURE_ROOT}' && export VIRTUAL_ENV='${TEST_TMPDIR}/other-venv' && ./04_load_requirements.sh"
  [ "$status" -eq 1 ]
...
EOF
  ...
  run bash -c "cd '${FIXTURE_ROOT}' && \
    export VIRTUAL_ENV=\"\$(cd '${FIXTURE_ROOT}/fixture-venv' && pwd -P)\" && \
    export PATH='${STUB_BIN}:'\${PATH} && \
    ./04_load_requirements.sh oops"
  [ "$status" -ne 0 ]
  [[ "$output" == *"Invalid parameter"* || "$output" == *"Error:"* ]]
}

@test "04_load_requirements is locked and excluded from automatic tag pairing" {
  #R030-T01 #R030-T02
  local real_script
  real_script="$(repo_root)/04_load_requirements.sh"
  [ -f "$real_script" ]
  grep -q "DO_NOT_MODIFY_THIS_FILE" "$real_script"
  grep -q "<AI_MODEL_INSTRUCTION>" "$real_script"
  run bash -c "cd '$(repo_root)' && ./tests/t04_run_requirements_traceability_tests.sh requirements/04_load_requirements-requirements.md 04_load_requirements.sh"
  [ "$status" -eq 0 ]
