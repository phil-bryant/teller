...
setup() {
  setup_shell_test
  create_repo_fixture
  copy_script_to_fixture "t15_verify_macos_crash_test.sh"
...
EOF
  ...
  run bash -c "cd '${TEST_TMPDIR}' && MACOS_UI_DIR='${FIXTURE_ROOT}/src/macos-ui' CRASH_REPORT_DIR='${report_dir}' STARTUP_WAIT_SECONDS=2 '${FIXTURE_ROOT}/t15_verify_macos_crash_test.sh'"
  [ "$status" -eq 0 ]
...
EOF
  ...
  run env MACOS_UI_DIR="${FIXTURE_ROOT}/src/macos-ui" \
    ...
    bash "${FIXTURE_ROOT}/t15_verify_macos_crash_test.sh"
  [ "$status" -eq 0 ]
  [[ "$output" == *"PLCrashReporter verification passed"* ]]
}
...
EOF
  ...
  run env MACOS_UI_DIR="${FIXTURE_ROOT}/src/macos-ui" bash "${FIXTURE_ROOT}/t15_verify_macos_crash_test.sh"
  [ "$status" -eq 1 ]
...
EOF
  ...
  run env MACOS_UI_DIR="${FIXTURE_ROOT}/missing-ui" bash "${FIXTURE_ROOT}/t15_verify_macos_crash_test.sh"
  [ "$status" -eq 1 ]
...
@test "remains standalone and is not chained by numbered runners" {
  #R040-T01
  run grep -E 'verify_macos_crash_test|CRASH_REPORTER_SMOKE' "${FIXTURE_ROOT}/t07_run_shell_unit_tests.sh"
  [ "$status" -ne 0 ]
