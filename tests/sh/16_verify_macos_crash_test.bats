...
EOF
  ...
  run bash -c "cd '${TEST_TMPDIR}' && MACOS_UI_DIR='${FIXTURE_ROOT}/macos-ui' CRASH_REPORT_DIR='${report_dir}' STARTUP_WAIT_SECONDS=2 '${FIXTURE_ROOT}/16_verify_macos_crash_test.sh'"
  [ "$status" -eq 0 ]
...
EOF
  ...
  run env MACOS_UI_DIR="${FIXTURE_ROOT}/macos-ui" bash "${FIXTURE_ROOT}/16_verify_macos_crash_test.sh"
  [ "$status" -eq 1 ]
