...
setup() {
  setup_shell_test
  create_repo_fixture
  copy_script_to_fixture "11_run_all_tests_parallel.sh"
...
@test "runs from repository root regardless of caller directory" {
  ...
  run bash -c "cd '${TEST_TMPDIR}' && PARALLEL_CHECKS_REPORT_DIR='${REPORT_DIR}' bash '${FIXTURE_ROOT}/11_run_all_tests_parallel.sh'"
  [ "$status" -eq 0 ]
...
@test "discovers only numbered test scripts and excludes self" {
  ...
  write_child_stub "06_deploy_database.sh" 'echo "non-test-script-ran" >> "'"${CALLS_LOG}"'"; exit 0'
  ...
  [[ "$output" != *"✅ PASS: 06_deploy_database.sh"* ]]
  [[ "$output" != *"✅ PASS: 11_run_all_tests_parallel.sh"* ]]
...
@test "rejects concurrent orchestrator runs with an active lock" {
  ...
  env PARALLEL_CHECKS_REPORT_DIR="${REPORT_DIR}" \
    bash "${FIXTURE_ROOT}/11_run_all_tests_parallel.sh" > "${TEST_TMPDIR}/first-run.log" 2>&1 &
  first_pid="$!"
  sleep 0.2
...
@test "--no-ui skips the macOS UI regression lane" {
  ...
  run env PARALLEL_CHECKS_REPORT_DIR="${REPORT_DIR}" \
    bash "${FIXTURE_ROOT}/11_run_all_tests_parallel.sh" --no-ui
  [ "$status" -eq 0 ]
...
@test "rejects unknown CLI arguments with usage guidance" {
  ...
  run env PARALLEL_CHECKS_REPORT_DIR="${REPORT_DIR}" \
    bash "${FIXTURE_ROOT}/11_run_all_tests_parallel.sh" --nope
  [ "$status" -eq 2 ]
