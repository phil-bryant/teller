...
setup() {
  setup_shell_test
  create_repo_fixture
  copy_script_to_fixture "07_verify_deploy_database.sh"
...
EOF
  chmod +x "${STUB_BIN}/psql"
  run env TELLER_DB_PASSWORD=pw zsh "${FIXTURE_ROOT}/07_verify_deploy_database.sh"
  [ "$status" -ne 0 ]
}
...
@test "psql command includes custom host and port from env" {
  ...
  run env TELLER_DB_HOST=custom.local TELLER_DB_PORT=15432 TELLER_DB_PASSWORD=pw TELLER_DB_NAME=d TELLER_DB_USER=u \
    zsh "${FIXTURE_ROOT}/07_verify_deploy_database.sh"
  [ "$status" -eq 0 ]
...
@test "uses 1psa when teller db password is unset" {
  ...
  : > "${PSQL_LOG}"
  make_psql_happy
  run env -u TELLER_DB_PASSWORD zsh "${FIXTURE_ROOT}/07_verify_deploy_database.sh"
  [ "$status" -eq 0 ]
  [[ "$(cat "${PSQL_LOG}")" == *"psql "* ]]
}
...
@test "fails when classification FK is missing ON DELETE CASCADE" {
  ...
  : > "${PSQL_LOG}"
  make_psql_happy
  run env TELLER_DB_PASSWORD=pw FK_BROKEN=1 zsh "${FIXTURE_ROOT}/07_verify_deploy_database.sh"
  [ "$status" -ne 0 ]
  [[ "$output" == *"❌ FAIL:"* ]]
