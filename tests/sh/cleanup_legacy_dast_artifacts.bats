...
@test "wraps deletes in transaction and FK-safe sequence" {
  ...
  [ "$status" -eq 0 ]
  run grep "DELETE FROM classy.transaction_nys_snw_category" "$(sql_file)"
  [ "$status" -eq 0 ]
  run grep "DELETE FROM classy.nys_snw_category" "$(sql_file)"
  [ "$status" -eq 0 ]
  run grep "DELETE FROM matchy.transaction_email_match" "$(sql_file)"
  [ "$status" -eq 0 ]
