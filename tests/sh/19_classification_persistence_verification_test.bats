...
# Requirement test-case tags for requirements/19_classification_persistence_verification_test-requirements.md
...
# Traceability numbered tags for requirements/19_classification_persistence_verification_test-requirements.md
...
setup() {
  setup_shell_test
  create_repo_fixture
  copy_script_to_fixture "19_classification_persistence_verification_test.sh"
...
EOF
  ...
  run env TELLER_DB_PASSWORD=pw TXN_ID=txn1 CATEGORY_ID=7 \
    zsh "${FIXTURE_ROOT}/19_classification_persistence_verification_test.sh" --require-env-ids
  [ "$status" -ne 0 ]
}
...
@test "auto-resolves posted transaction and category" {
  ...
  : > "${PSQL_16}"; : > "${CURL_LOG16}"
  write_psql16
  run env -u TXN_ID -u CATEGORY_ID TELLER_DB_PASSWORD=pw zsh "${FIXTURE_ROOT}/19_classification_persistence_verification_test.sh"
  [ "$status" -eq 0 ]
  [[ "$output" == *"✅ PASS:"* ]]
}
...
@test "resolves empty password with 1psa" {
  ...
  : > "${PSQL_16}"; : > "${CURL_LOG16}"
  write_psql16
  run env -u TELLER_DB_PASSWORD -u TXN_ID -u CATEGORY_ID zsh "${FIXTURE_ROOT}/19_classification_persistence_verification_test.sh"
  [ "$status" -eq 0 ]
}
...
@test "api json uses transaction and category" {
  ...
  : > "${PSQL_16}"
  write_psql16
  run env TELLER_DB_PASSWORD=pw TXN_ID=txn1 CATEGORY_ID=7 zsh "${FIXTURE_ROOT}/19_classification_persistence_verification_test.sh" --require-env-ids
  [ "$status" -eq 0 ]
...
EOF
  chmod +x "${STUB_BIN}/curl"
  write_psql16
  run env CLASSIFICATION_PERSISTENCE_START_API=false TELLER_DB_PASSWORD=pw TXN_ID=txn1 CATEGORY_ID=7 zsh "${FIXTURE_ROOT}/19_classification_persistence_verification_test.sh" --require-env-ids
  [ "$status" -ne 0 ]
  [[ "$output" == *"❌ FAIL:"* ]]
