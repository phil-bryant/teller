...
# Requirement test-case tags for requirements/14_verify_macos_crash_test-requirements.md
# #R015-T02: Traceability anchor.
# #R020-T02: Traceability anchor.
# #R030-T02: Traceability anchor.

# Traceability numbered tags for requirements/14_verify_macos_crash_test-requirements.md
...
setup() {
  setup_shell_test
  create_repo_fixture
  copy_script_to_fixture "14_verify_macos_crash_test.sh"
  copy_script_to_fixture "09_run_shell_unit_tests.sh"
  copy_script_to_fixture "13_run_macos_ui_regression_tests.sh"
...
EOF
  ...
  run bash -c "cd '${TEST_TMPDIR}' && MACOS_UI_DIR='${FIXTURE_ROOT}/macos-ui' CRASH_REPORT_DIR='${report_dir}' STARTUP_WAIT_SECONDS=2 '${FIXTURE_ROOT}/14_verify_macos_crash_test.sh'"
  [ "$status" -eq 0 ]
...
EOF
  ...
  run env MACOS_UI_DIR="${FIXTURE_ROOT}/macos-ui" bash "${FIXTURE_ROOT}/14_verify_macos_crash_test.sh"
  [ "$status" -eq 1 ]
...
EOF
  ...
  run env MACOS_UI_DIR="${FIXTURE_ROOT}/missing-ui" bash "${FIXTURE_ROOT}/14_verify_macos_crash_test.sh"
  [ "$status" -eq 1 ]
...
@test "remains standalone and is not chained by numbered runners" {
  ...
  run grep -E 'verify_macos_crash_test|CRASH_REPORTER_SMOKE' "${FIXTURE_ROOT}/13_run_macos_ui_regression_tests.sh"
  [ "$status" -ne 0 ]
}
