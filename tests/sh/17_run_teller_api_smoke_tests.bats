...
# Requirement test-case tags for requirements/17_run_teller_api_smoke_tests-requirements.md
# #R005-T02: Traceability anchor.
# #R010-T02: Traceability anchor.

# Traceability numbered tags for requirements/17_run_teller_api_smoke_tests-requirements.md
...
EOF
  ...
  run bash -c "cd '${FIXTURE_ROOT}' && DEPENDENCY_CHECK_PYTHON='${FIXTURE_ROOT}/teller-venv/bin/python' TELLER_SMOKE_INSTITUTION_ID='chase' ./17_run_teller_api_smoke_tests.sh"
  [ "$status" -eq 0 ]
  calls="$(<"${CALLS_LOG}")"
...
@test "fails fast for non-executable explicit interpreter path" {
  #R005
  run bash -c "cd '${FIXTURE_ROOT}' && DEPENDENCY_CHECK_PYTHON=./missing-python ./17_run_teller_api_smoke_tests.sh"
  [ "$status" -eq 1 ]
