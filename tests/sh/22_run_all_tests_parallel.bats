...
# Requirement test-case tags for requirements/22_run_all_tests_parallel-requirements.md
...
# Traceability numbered tags for requirements/22_run_all_tests_parallel-requirements.md
...
setup() {
  setup_shell_test
  create_repo_fixture
  copy_script_to_fixture "22_run_all_tests_parallel.sh"
...
@test "reports pass for all fourteen checks when every child succeeds" {
  #R001 #R025 #R030
  write_all_child_stubs 'echo "stub ${BASH_SOURCE[0]##*/}"; exit 0'

  run env PARALLEL_CHECKS_REPORT_DIR="${REPORT_DIR}" \
    bash "${FIXTURE_ROOT}/22_run_all_tests_parallel.sh"
  [ "$status" -eq 0 ]
  [[ "$output" == *"✅ PASS: all parallel checks succeeded (14/14)"* ]]
  [[ "$output" == *"Timing: wall "* ]]
...
@test "runs from repository root regardless of caller directory" {
  ...
  run bash -c "cd '${TEST_TMPDIR}' && PARALLEL_CHECKS_REPORT_DIR='${REPORT_DIR}' bash '${FIXTURE_ROOT}/22_run_all_tests_parallel.sh'"
  [ "$status" -eq 0 ]
...
@test "discovers only numbered test scripts and excludes self" {
  ...
  run env PARALLEL_CHECKS_REPORT_DIR="${REPORT_DIR}" \
    bash "${FIXTURE_ROOT}/22_run_all_tests_parallel.sh"
  [ "$status" -eq 0 ]
  [[ "$output" == *"▶ Starting parallel checks (14 scripts)..."* ]]
  [[ "$output" != *"✅ PASS: 07_deploy_database.sh"* ]]
  [[ "$output" != *"✅ PASS: 22_run_all_tests_parallel.sh"* ]]
...
@test "renders intermediate progress before all checks complete" {
  ...
  [[ "$output" == *"Progress: [0/14 (0%)]"* ]]
  [[ "$output" =~ Progress:\ \[([1-9]|1[0-3])/14\ \([0-9]+%\)\] ]]
}
...
@test "prints final 100 percent progress before overall summary" {
  ...
  before_overall="${output%%✅ PASS: all parallel checks succeeded (14/14)*}"
  [[ "$before_overall" == *"Progress: [14/14 (100%)]"* ]]
  [[ "$output" == *"✅ PASS: all parallel checks succeeded (14/14)"* ]]
}
...
@test "waits for all checks and reports a single failed child" {
  #R020 #R025 #R030 #R035
  write_all_child_stubs 'exit 0'
  write_child_stub "10_run_python_unit_tests.sh" 'echo "unit-tests-failed"; exit 1'

  run env PARALLEL_CHECKS_REPORT_DIR="${REPORT_DIR}" \
    bash "${FIXTURE_ROOT}/22_run_all_tests_parallel.sh"
  [ "$status" -eq 1 ]
  [[ "$output" == *"❌ FAIL: 10_run_python_unit_tests.sh (exit 1,"* ]]
  [[ "$output" == *"see ${REPORT_DIR}/09_run_unit_tests.log"* ]]
  [[ "$output" == *"✅ PASS: 00_run_requirements_traceability_tests.sh"* ]]
  [[ "$output" == *"❌ FAIL: parallel checks: 13/14 passed"* ]]
  grep -q 'unit-tests-failed' "${REPORT_DIR}/10_run_python_unit_tests.log"
}
...
@test "rejects concurrent orchestrator runs with an active lock" {
  ...
  env PARALLEL_CHECKS_REPORT_DIR="${REPORT_DIR}" \
    bash "${FIXTURE_ROOT}/22_run_all_tests_parallel.sh" > "${TEST_TMPDIR}/first-run.log" 2>&1 &
  first_pid="$!"
  sleep 0.2
...
@test "reclaims stale lock file and succeeds" {
  #R050
  write_all_child_stubs 'exit 0'
  printf '%s\n' 999999 > "${FIXTURE_ROOT}/.22_run_all_tests_parallel.lock"

  run env PARALLEL_CHECKS_REPORT_DIR="${REPORT_DIR}" \
    bash "${FIXTURE_ROOT}/22_run_all_tests_parallel.sh"
  [ "$status" -eq 0 ]
  [ ! -f "${FIXTURE_ROOT}/.22_run_all_tests_parallel.lock" ]
}
