...
# Requirement test-case tags for requirements/20_run_dynamic_security_tests-requirements.md

# Traceability numbered tags for requirements/20_run_dynamic_security_tests-requirements.md
...
write_dast_13_stub() {
  local root="$1"
  cat > "${root}/18_run_classification_api.py" <<'PYS'
#!/usr/bin/env python3
import http.server
import os
import socketserver
...
copy_dast_project_files() {
  create_repo_fixture
  copy_script_to_fixture "20_run_dynamic_security_tests.sh"
...
@test "prints DAST startup banner" {
  ...
  run env RUN_DAST=false \
    bash "${FIXTURE_ROOT}/20_run_dynamic_security_tests.sh"
  [ "$status" -eq 0 ]
...
@test "category integrity gate asserts seed protection invariants" {
  setup_shell_test
  copy_dast_project_files
  run /usr/bin/python3 - <<'PY' "${FIXTURE_ROOT}/20_run_dynamic_security_tests.sh"
import pathlib
import sys
