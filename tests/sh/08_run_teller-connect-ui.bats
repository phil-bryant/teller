...
setup() {
  setup_shell_test
  create_repo_fixture
  copy_script_to_fixture "08_run_teller-connect-ui.sh"
...
@test "default mode launches capture server in manage mode" {
  run bash "${FIXTURE_ROOT}/08_run_teller-connect-ui.sh"
  [ "$status" -eq 0 ]
  calls="$(<"${CALLS_LOG}")"
...
@test "list mode prints discovered contexts" {
  ...
  run bash "${FIXTURE_ROOT}/08_run_teller-connect-ui.sh" --list
  [ "$status" -eq 0 ]
  [[ "$output" == *"source"* ]]
  [[ "$output" == *"enr_default"* ]]
  [[ "$output" == *"bank_a"* ]]
...
@test "delete mode removes only selected context using --yes" {
  ...
  run bash "${FIXTURE_ROOT}/08_run_teller-connect-ui.sh" --delete --institution_id bank_a --yes
  [ "$status" -eq 0 ]
...
@test "positional token writes auth_token.json with restrictive mode" {
  run bash "${FIXTURE_ROOT}/08_run_teller-connect-ui.sh" token_direct_value
  [ "$status" -eq 0 ]
...
@test "selector operations fail clearly when selector is ambiguous" {
  ...
  run bash "${FIXTURE_ROOT}/08_run_teller-connect-ui.sh" --delete --enrollment_id enr_shared
  [ "$status" -eq 0 ]
  [[ "$output" == *"ambiguous"* ]]
}
