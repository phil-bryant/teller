...
setup() {
  setup_shell_test
  create_repo_fixture
  copy_script_to_fixture "09_verify_updated_at_trigger_coverage.sh"
...
EOF
  chmod +x "${STUB_BIN}/psql"
  run env TELLER_DB_PASSWORD=pw zsh "${FIXTURE_ROOT}/09_verify_updated_at_trigger_coverage.sh"
  [ "$status" -ne 0 ]
}
...
@test "psql includes host from teller db env" {
  ...
  run env TELLER_DB_HOST=db.example TELLER_DB_PORT=1234 TELLER_DB_NAME=n TELLER_DB_USER=u TELLER_DB_PASSWORD=pw \
    zsh "${FIXTURE_ROOT}/09_verify_updated_at_trigger_coverage.sh"
  [ "$status" -eq 0 ]
...
@test "uses 1psa when teller db password is unset" {
  ...
  : > "${PSQL_LOG_08}"
  write_psql_08
  run env -u TELLER_DB_PASSWORD zsh "${FIXTURE_ROOT}/09_verify_updated_at_trigger_coverage.sh"
  [ "$status" -eq 0 ]
}
...
@test "lists missing table when trigger coverage is incomplete" {
  ...
  : > "${PSQL_LOG_08}"
  write_psql_08
  run env TELLER_DB_PASSWORD=pw GAPS=1 zsh "${FIXTURE_ROOT}/09_verify_updated_at_trigger_coverage.sh"
  [ "$status" -ne 0 ]
