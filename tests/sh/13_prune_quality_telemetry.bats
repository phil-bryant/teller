...
setup() {
  setup_shell_test
  create_repo_fixture
  copy_script_to_fixture "13_prune_quality_telemetry.sh"
}
...
@test "prunes oldest lane summaries and keeps newest by count" {
  ...
  run bash -c "cd '${FIXTURE_ROOT}/tmp-run' && env QUALITY_LANE_SUMMARY_KEEP=2 bash '${FIXTURE_ROOT}/13_prune_quality_telemetry.sh'"
...
@test "succeeds without changes when lane summaries are absent" {
  ...
  run env QUALITY_TELEMETRY_DIR="${telemetry_dir}" \
    bash "${FIXTURE_ROOT}/13_prune_quality_telemetry.sh"
