...
# Requirement test-case tags for requirements/12_run_sql_unit_tests-requirements.md
...
setup() {
  setup_shell_test
  create_repo_fixture
  copy_script_to_fixture "12_run_sql_unit_tests.sh"
...
@test "runs from repository root regardless of caller cwd" {
  #R001
  run bash -c "cd '${TEST_TMPDIR}' && '${FIXTURE_ROOT}/12_run_sql_unit_tests.sh'"
  [ "$status" -eq 0 ]
}

@test "enables only the expected lane" {
  #R005
  run bash -c "cd '${FIXTURE_ROOT}' && ./12_run_sql_unit_tests.sh"
  [ "$status" -eq 0 ]
  calls="$(<"${CALLS_LOG}")"
