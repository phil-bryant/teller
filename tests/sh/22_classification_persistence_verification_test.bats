...
setup() {
  setup_shell_test
  create_repo_fixture
  copy_script_to_fixture "22_classification_persistence_verification_test.sh"
...
EOF
  chmod +x "${STUB_BIN}/curl"
  write_psql16
  run env CLASSIFICATION_PERSISTENCE_START_API=false TELLER_DB_PASSWORD=pw TXN_ID=txn1 CATEGORY_ID=7 zsh "${FIXTURE_ROOT}/22_classification_persistence_verification_test.sh" --require-env-ids
  [ "$status" -ne 0 ]
  [[ "$output" == *"❌ FAIL:"* ]]
