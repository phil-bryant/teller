...
# Requirement test-case tags for requirements/09_run_shell_unit_tests-requirements.md
...
@test "runs from repository root regardless of caller cwd" {
  #R001
  run bash -c "cd '${TEST_TMPDIR}' && '${FIXTURE_ROOT}/09_run_shell_unit_tests.sh'"
  [ "$status" -eq 0 ]
  last_line="${output##*$'\n'}"
...
@test "enables only the expected lane" {
  #R005
  run bash -c "cd '${FIXTURE_ROOT}' && ./09_run_shell_unit_tests.sh"
  [ "$status" -eq 0 ]
  calls="$(<"${CALLS_LOG}")"
...
@test "prints final failure marker on failure" {
  #R006-T02
  run bash -c "cd '${FIXTURE_ROOT}' && FORCE_FAIL=true ./09_run_shell_unit_tests.sh"
  [ "$status" -eq 1 ]
  last_line="${output##*$'\n'}"
