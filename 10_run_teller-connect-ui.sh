...
ensure_unique_suffix() {
    #region agent log
    debug_log "H1" "10_run_teller-connect-ui.sh:ensure_unique_suffix" "entry" "raw_arg=$1"
    #endregion
...
usage() {
    ...
    echo "Usage:"
    echo "  ./10_run_teller-connect-ui.sh                 # default: no copy/paste flow"
    echo "  ./10_run_teller-connect-ui.sh --connect      # explicit no copy/paste flow"
    echo "  ./10_run_teller-connect-ui.sh --manual"
    echo "  ./10_run_teller-connect-ui.sh [token_xxx]"
    echo "  ./10_run_teller-connect-ui.sh --clipboard"
    echo "  ./10_run_teller-connect-ui.sh --list"
    echo "  ./10_run_teller-connect-ui.sh --delete --institution_id <id>|--enrollment_id <id> [--yes]"
    echo "  ./10_run_teller-connect-ui.sh --reconnect --institution_id <id>|--enrollment_id <id>"
    echo "  ./10_run_teller-connect-ui.sh --add"
    echo ""
...
verify_accounts_access() {
    ...
    if [ "$status" = "200" ]; then
        ...
        if [[ "$error_code" == enrollment.disconnected* ]] && [ -z "$ENROLLMENT_ID" ]; then
            ...
            echo "  ENROLLMENT_ID=enr_xxx ./10_run_teller-connect-ui.sh"
        fi
        echo "Response:"
        cat "$response_file"
    fi
...
EOF
        ...
        --add)
            ...
            #region agent log
            debug_log "H2" "10_run_teller-connect-ui.sh:main:add" "temp_output_paths" "token_out=$token_out enrollment_out=$enrollment_out"
            #endregion
            run_connect_capture "" "$token_out" "$enrollment_out" "capture"
            institution="$(infer_institution_id_from_token_file "$token_out")"
            #region agent log
            debug_log "H3" "10_run_teller-connect-ui.sh:main:add" "institution_from_identity" "institution=$institution"
            #endregion
            [ -n "$institution" ] || institution="$(tr -d '\r\n' < "$enrollment_out")"
            #region agent log
            debug_log "H4" "10_run_teller-connect-ui.sh:main:add" "institution_after_fallback" "institution=$institution"
            #endregion
            institution="$(sanitize_suffix "$institution")"
            #region agent log
            debug_log "H5" "10_run_teller-connect-ui.sh:main:add" "institution_after_sanitize" "institution=$institution"
            #endregion
