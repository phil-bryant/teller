...
class TellerAPIError(Exception):
    #R040: Preserve upstream API status/code/message metadata in a typed error object.
...
class TellerAPIClient:
    ...
    #R005: Initialize client auth context from explicit token override or ~/.teller defaults.
    ...
    #R040: Parse Teller API error payloads into stable code/message values.
    ...
    def _repair_enrollment(self) -> bool:
        #R010: Enrollment repair is performed manually via the classy macOS Connect UI
        #R010: (the classification UI was extracted to the sibling classy repo).
        ...
        if enrollment_id:
            raise TellerAPIError(
                message=(
                    "Enrollment disconnected. Reconnect via the classy macOS app "
                    "(../classy/10_run_classification_macos_ui.sh, Connect tab), then rerun this script."
                ),
                code="enrollment.disconnected.manual_repair_required",
                status_code=0,
            )
        log.warning("Cannot repair: no enrollment ID", path=str(enrollment_id_file))
        return repaired
...
#R015: Resolve transaction pagination page cap from environment with safe fallback.
...
#R020: Read context token from auth token JSON payload when present.
...
#R020: Read optional plain-text enrollment context value from disk.
...
#R020: Load enrollment contexts from enrollments metadata file.
...
#R020: Discover enrollment contexts from suffixed auth token files.
...
#R020: Infer enrollment IDs from persisted account rows when explicit contexts are absent.
...
#R020: Deduplicate merged enrollment contexts by enrollment ID or institution/token fingerprint.
...
#R025: Fetch identities, transactions, and balances for one enrollment context scope.
...
def main():
    ...
    try:
        ...
        if exc.code.startswith("enrollment.disconnected"):
            print("Reconnect via the classy macOS app (../classy/10_run_classification_macos_ui.sh, Connect tab) to repair.")
        sys.exit(1)
